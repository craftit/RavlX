...
namespace RavlImageN {
  ...
  template<class DataT>
  void DrawFrame(Array2dC<DataT> &dat,const DataT &value,int width,const IndexRange2dC &outerRect) {
    IndexRange2dC innerRect = outerRect.Shrink(width);
    IndexRange2dC outerClipped = outerRect;
	...
	for(;it1 != eor;it1++,it2++) {
	  ...
	  *it2 = value;
      ...
      IndexRange2dC r1(Index2dC(innerRect.TRow(),outerRect.LCol()),Index2dC(innerRect.BRow(),innerRect.LCol()-1));
      r1.ClipBy(dat.Frame());
      if(r1.Area() > 0) {
	for (Array2dIterC<DataT> it(dat,r1); it; it++) 
	  *it = value;
      }
      IndexRange2dC r2(Index2dC(innerRect.TRow(),innerRect.RCol()+1),Index2dC(innerRect.BRow(),outerRect.RCol()));
      r2.ClipBy(dat.Frame());
      if(r2.Area() > 0) {
